...
module.exports = {
    ...
    create(context) {
        // Parse options.
        const keys = context.options[0] || [];
        const order = context.options[1] || 'asc';
        const options = context.options[2];
        const insensitive = (options && options.caseSensitive) === false;
        const natual = Boolean(options && options.natural);
        const isValidOrderFallback = isValidOrders[order + (insensitive ? 'I' : '') + (natual ? 'N' : '')];

        const isValidOrder = (a, b) => {
            let i1 = keys.indexOf(a);
            let i2 = keys.indexOf(b);
            if (i1 === -1) {
            ...
            return isValidOrderFallback(a, b);
        };
            ...
            Property(node) {
                ...
                if (!isValidOrder(prevName, thisName)) {
                    context.report({
                        ...
                        loc: node.key.loc,
                        message:
                            "Expected object keys to be ordered correctly. '{{thisName}}' should be before '{{prevName}}'.",
                        data: {
                            thisName,
                            prevName,
                        },
